...
export class PlayerStatisticsResolver {
  ...
  async findAll(@Args("paginationInput") paginationInput: PaginationGroupInput): Promise<PlayerStatistics[]> {
    return this.playerStatisticsService.findAll(paginationInput);
  }

  @Query(() => PlayerStatistics, { name: 'playerStatistic' })
  async findOne(@Args('id', { type: () => Int }) id: number): Promise<PlayerStatistics> {
    return this.playerStatisticsService.findOne(id);
  }

  @Mutation(() => PlayerStatistics)
  async updatePlayerStatistic(@Args('updatePlayerStatisticInput') updatePlayerStatisticInput: UpdatePlayerStatisticInput): Promise<PlayerStatistics> {
    return this.playerStatisticsService.update(updatePlayerStatisticInput.id, updatePlayerStatisticInput);
  }
